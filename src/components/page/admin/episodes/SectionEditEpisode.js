...
const SectionEditEpisode = (props) => {
    ...
    const fetchIniDetails = () => {
        ...
        setEpisode({
            ...episode,
            selectedType: {value: episode.type, label: episode.type === "cash" ? "نقدی" : "رایگان"},
            selectedCourse: {value: episode.course.id, label: episode.course.title}
        });

        axios.get(`${API_ADMIN}/editEpisode?api_token=${localStorage.getItem(APP_TOKEN_NAME)}`)
            .then(response => {
            ...
            case 'number':
                errors.number =
                    validator.isEmpty(value)
                        ? 'شماره ویدیو نمیتواند خالی باشد!'
                        : '';
                break;
    ...
    const handleChangeCourse = selectedOption => {
        setEpisode(prevState => ({...prevState, course_id: selectedOption.value, selectedCourse: selectedOption}))
    };

    const handleChangeType = selectedOption => {
        setEpisode(prevState => ({...prevState, type: selectedOption.value , selectedType: selectedOption}))
    };
    ...
    const submitData = () => {
        ...
        if (
            ...
            axios.post(`${API_ADMIN}/episode/${episode.id}/update`,
                {
                ...
                if (response.data.status === "success") {
                    swal("عملیات موفق", "ویدیو به درستی به روزرسانی شد.", "success")
...
export default SectionEditEpisode;